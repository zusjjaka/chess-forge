...
| `revision`         | `int`         | Техническая ревизия состояния дерева. Используется для optimistic locking и изменяется при изменении дерева. |
| `analytic_version` | `int`         | Версия полного состояния дерева, используемая для привязки аналитики.                                        |
...
Начальные значения нового репертуара:
...
`revision` используется только для конкурентных изменений и не является версией аналитики.

`analytic_version` увеличивается только при полном `PUT` дерева.
...
| `analytic_version` | `int`          | Версия ходов линии, используемая для определения актуальности аналитики этой конкретной линии. |
| `created_at`       | `timestamptz`  | Дата и время создания линии.                                                                   |
| `updated_at`       | `timestamptz`  | Дата и время последнего изменения линии.                                                       |

Начальное значение новой линии:

```text
analytic_version = 1
```

Изменение `tag` не изменяет `analytic_version`.

Изменение `moves` увеличивает `analytic_version` на `1`.
...
| `repertoire_revision` | `int`         | Ревизия репертуара на момент создания тренировочной сессии.          |
...
`repertoire_revision` позволяет определить, изменился ли репертуар после создания тренировочной сессии.

`selected_moves` сохраняет выбранный тренировочный путь, поэтому для продолжения уже созданной сессии Training Service не должен повторно получать эти данные из Repertoire Service.
...
| `repertoire_revision` | `int`         | Ревизия репертуара, использованная во время тренировки.                                        |
